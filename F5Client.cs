...
namespace Keyfactor.Extensions.Orchestrator.F5Orchestrator
{
    public class F5Client
    {
        ...
        public string AddEntry(string partition, string name, string b64Certificate, string certificatePassword, bool AutoGenerateAlias, string AutoGeneratedAliasFormat)
        {
            LogHandlerCommon.MethodEntry(logger, CertificateStore, "AddEntry");
            
            string newName = AutoGenerateAlias ? CalculateAlias(b64Certificate, certificatePassword, Enum.Parse<AliasFormatEnum>(AutoGeneratedAliasFormat)) : name;
            if (name != newName && CertificateExists(partition, newName))
                throw new Exception($"The certificate alias '{newName}' generated from {AutoGeneratedAliasFormat} already exists in partition '{partition}'. Please use a different alias or disable auto-generation of the alias.");

            AddEntry(partition, newName, b64Certificate, certificatePassword);
        ...
        public string ReplaceEntry(string partition, string name, string b64Certificate, string certificatePassword, bool AutoGenerateAlias, string AutoGeneratedAliasFormat, bool KeepSameAliasOnRenewal)
        {
            LogHandlerCommon.MethodEntry(logger, CertificateStore, "ReplaceEntry");

            string newName = AutoGenerateAlias && !KeepSameAliasOnRenewal ? CalculateAlias(b64Certificate, certificatePassword, Enum.Parse<AliasFormatEnum>(AutoGeneratedAliasFormat)) : name;
            if (name != newName && CertificateExists(partition, newName))
                throw new Exception($"The certificate alias '{newName}' generated from {AutoGeneratedAliasFormat} already exists in partition '{partition}'. Please use a different alias or disable auto-generation of the alias.");

            if (name == newName)
                ReplaceEntry(partition, newName, b64Certificate, certificatePassword);
            else
                AddEntry(partition, newName, b64Certificate, certificatePassword);
        ...
        private string CalculateAlias(string certContents, string password, AliasFormatEnum autoGeneratedAliasFormat)
        {
            LogHandlerCommon.MethodEntry(logger, CertificateStore, "CalculateAlias");

            CertificateConverter converter = CertificateConverterFactory.FromDER(Convert.FromBase64String(certContents), password);
            X509Certificate2 certificate = converter.ToX509Certificate2(password);

            string alias = string.Empty;
            switch (autoGeneratedAliasFormat)
            {
                case AliasFormatEnum.CN:
                    alias = certificate.GetNameInfo(X509NameType.SimpleName, false);
                    break;
                case AliasFormatEnum.SerialNumber:
                    alias = certificate.SerialNumber;
                    break;
                case AliasFormatEnum.Thumbprint:
                    alias = certificate.Thumbprint;
                    break;
                case AliasFormatEnum.CN_SerialNumber:
                    alias = $"{certificate.GetNameInfo(X509NameType.SimpleName, false)}-{certificate.SerialNumber}";
                    break;
                case AliasFormatEnum.CN_Thumbprint:
                    alias = $"{certificate.GetNameInfo(X509NameType.SimpleName, false)}-{certificate.Thumbprint}";
                    break;
                default:
                    throw new Exception($"Unsupported auto-generated alias format '{autoGeneratedAliasFormat.ToString()}'");
            }

            LogHandlerCommon.MethodExit(logger, CertificateStore, "CalculateAlias");
            return alias;
        }
