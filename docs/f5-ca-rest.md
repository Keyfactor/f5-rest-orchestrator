...
The F5 CA Profiles REST Certificate Store Type is designed to manage certificate authority (CA) bundles on F5 devices. These bundles are essential for establishing a chain of trust, allowing certificates to be validated against trusted CAs. This store type supports various job types, including Discovery, Inventory, and Management (Add and Remove), enabling comprehensive management of CA bundles directly from Keyfactor Command.

The F5 CA Profiles REST represents the CA certificates stored on F5 devices, which are used to verify the authenticity of other certificates. An important consideration is that, by convention, certificates in CA Bundles are presumed not to have private keys. This assumption is critical because F5's API does not provide a mechanism to confirm the presence of private keys.

This certificate store type does not use an SDK and instead interacts with the F5 devices through their REST APIs. A key limitation to be aware of is this store type's management functionality, particularly when adding certificates. Unlike other store types, CA Bundle certificates are treated as not having private keys, which could confuse users when managing them.

Users should be attentive to configuring the certificate store accurately, especially ensuring correct partition names and the `UseSSL` setting. Misconfigurations in these fields might lead to operational issues. Additionally, understanding the nuances of how CA certificates are managed without private keys will help avoid misinterpretations during inventory and management jobs.
...
The recommended method for creating the `F5-CA-REST` Certificate Store Type is to use [kfutil](https://github.com/Keyfactor/kfutil). After installing, use the following command to create the `` Certificate Store Type:
...
After creating the `F5-CA-REST` Certificate Store Type and installing the F5 Universal Orchestrator extension, you can create new [Certificate Stores](https://software.keyfactor.com/Core-OnPrem/Current/Content/ReferenceGuide/Certificate%20Stores.htm?Highlight=certificate%20store) to manage certificates in the remote platform.
