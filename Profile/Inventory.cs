    ...
    public class Inventory : InventoryBase
    {
        ...
        public override JobResult ProcessJob(InventoryJobConfiguration config, SubmitInventoryUpdate submitInventory)
        {
            ...
            try
            {
                ...
                string profileName = profileStorePath.ProfileName;
                string profileEndpoint = F5Client.GetProfileEndpoint(profileStorePath.ProfileType);

                LogHandlerCommon.Debug(logger, JobConfig.CertificateStoreDetails, $"Getting inventory for profile '{profileName}' in '{config.CertificateStoreDetails.StorePath}'");
                inventory = f5.GetProfileCertificateInventory(partition, profileEndpoint, profileName);
