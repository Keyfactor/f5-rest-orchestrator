...
The F5 WS Profiles REST Certificate Store Type is intended to handle Web Server device certificates on F5 systems. This store type plays a critical role in managing certificates that ensure secure web server communication, enabling SSL/TLS encryption. The F5 WS Profiles REST Certificate Store Type allows inventory and management tasks, including the addition of new certificates by replacing or renewing existing ones.

This store type represents the Web Server device certificates utilized by F5 devices, which are essential for securing communications through HTTPS. A significant aspect to note is the assumption that these certificates have associated private keys, even though Keyfactor Command will not retrieve these keys during inventory jobs. This is a necessary convention as F5's API does not provide a way to verify the presence of private keys.

The certificate store type does not rely on an SDK; instead, it performs operations through the REST APIs offered by F5. One important caveat is that management actions like adding a certificate will only replace or renew existing certificates, rather than adding completely new ones. This behavior is critical for users to understand to avoid unintended outcomes when performing certificate management.

Users should also be aware of the necessity to correctly specify the partition name on the F5 device and ensure that `UseSSL` is selected during configuration. Failing to adhere to these requirements might lead to configuration and operational issues. Additionally, setting the `Primary Node Online Required` might be necessary if the environment has high availability requirements, as this will ensure that certificates are only managed on active primary nodes.
...
The recommended method for creating the `F5-WS-REST` Certificate Store Type is to use [kfutil](https://github.com/Keyfactor/kfutil). After installing, use the following command to create the `` Certificate Store Type:
...
After creating the `F5-WS-REST` Certificate Store Type and installing the F5 Universal Orchestrator extension, you can create new [Certificate Stores](https://software.keyfactor.com/Core-OnPrem/Current/Content/ReferenceGuide/Certificate%20Stores.htm?Highlight=certificate%20store) to manage certificates in the remote platform.
