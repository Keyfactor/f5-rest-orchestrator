    ...
    public class Management : ManagementBase
    {
        ...
        private void PerformAddJob(F5Client f5, string partition, string certificatePassword)
        {
            ...
            if (certificateExists)
            {
                ...
                f5.ReplaceEntry(partition, name, certContents, certificatePassword, AutoGenerateAlias, AutoGeneratedAliasFormat, KeepSameAliasOnRenewal);
            }
            else
            {
                ...
                f5.AddEntry(partition, name, certContents, certificatePassword, AutoGenerateAlias, AutoGeneratedAliasFormat);
            }

            LogHandlerCommon.Debug(logger, JobConfig.CertificateStoreDetails, $"Binding '{name}' to profile '{ProfileName}'");
            f5.BindCertificateToProfile(partition, ProfileEndpoint, ProfileName, name, certificatePassword, certificateExists);
